...
describe("PolicyVoter Tests", function () {
  beforeEach(async function () {
    ...
    await pv.registerAddress(acc3.address, true, 1);
  });
  ...
  it("can vote or unvote a removed from blacklisted policy", async function () {
    ...
    await expect(pv.connect(acc1).vote("a1b2c3")).to.be.revertedWith(
      "policy is blacklisted"
    );
    await expect(pv.connect(acc2).unVote("a1b2c3")).to.be.revertedWith(
      "policy is blacklisted"
    );
    await waitForSeconds(31);
    await pv.setPolicyBlacklist(
      "a1b2c3",
      false,
      "policy removed from blacklist"
    );
